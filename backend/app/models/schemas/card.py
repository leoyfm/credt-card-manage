...
class BankBase(BaseModel):
    """银行基础模型"""
    bank_code: str = Field(..., max_length=20, description="银行代码", example="CMB")
    bank_name: str = Field(..., max_length=100, description="银行名称", example="招商银行")
    bank_logo: Optional[str] = Field(None, max_length=500, description="银行logo", example="https://example.com/cmb-logo.png")
    is_active: bool = Field(True, description="是否激活", example=True)
    sort_order: int = Field(0, description="排序", example=1)
...
class BankUpdate(BaseModel):
    """更新银行请求模型"""
    bank_name: Optional[str] = Field(None, max_length=100, description="银行名称", example="招商银行")
...
class CreditCardBase(BaseModel):
    """信用卡基础模型"""
    card_name: str = Field(..., max_length=100, description="卡片名称", example="招商银行信用卡")
    card_number: str = Field(..., max_length=100, description="卡号", example="6225********1234")
    card_type: str = Field("credit", max_length=20, description="卡片类型", example="credit")
    card_network: Optional[str] = Field(None, max_length=20, description="卡组织", example="VISA")
    card_level: Optional[str] = Field(None, max_length=20, description="卡片等级", example="白金卡")
    credit_limit: Decimal = Field(..., ge=0, description="信用额度", example=50000.00)
    expiry_month: int = Field(..., ge=1, le=12, description="有效期月份", example=12)
    expiry_year: int = Field(..., ge=2024, description="有效期年份", example=2027)
    ...
    annual_fee: Decimal = Field(0, ge=0, description="年费金额", example=300.00)
    fee_waivable: bool = Field(False, description="年费是否可减免", example=True)
    fee_auto_deduct: bool = Field(False, description="是否自动扣费", example=False)
    fee_due_month: Optional[int] = Field(None, ge=1, le=12, description="年费到期月份", example=12)
    features: List[str] = Field(default_factory=list, description="特色功能", example=["积分兑换", "免费洗车"])
    points_rate: Decimal = Field(1.00, ge=0, description="积分倍率", example=1.50)
    cashback_rate: Decimal = Field(0.00, ge=0, le=100, description="返现比例", example=1.00)
...
class CreditCardCreate(CreditCardBase):
    ...
    bank_name: Optional[str] = Field(None, max_length=100, description="银行名称（如果不提供bank_id）", example="招商银行")
...
class CreditCardResponse(CreditCardBase):
    ...
    bank_id: Optional[UUID] = Field(None, description="银行ID")
    available_limit: Optional[Decimal] = Field(None, description="可用额度", example=45000.00)
    used_limit: Decimal = Field(0, description="已用额度", example=5000.00)
    status: str = Field("active", description="状态", example="active")
    created_at: datetime = Field(..., description="创建时间")
    updated_at: datetime = Field(..., description="更新时间")
    ...
    # 计算属性
    expiry_display: Optional[str] = Field(None, description="有效期显示", example="12/27")
    is_expired: Optional[bool] = Field(None, description="是否已过期", example=False)
    credit_utilization_rate: Optional[float] = Field(None, description="信用额度使用率", example=10.0)
...
class CreditCardSummary(BaseModel):
    ...
    active_cards: int = Field(..., description="激活卡片数", example=4)
    total_credit_limit: Decimal = Field(..., description="总信用额度", example=250000.00)
    total_used_limit: Decimal = Field(..., description="总已用额度", example=25000.00)
    total_available_limit: Decimal = Field(..., description="总可用额度", example=225000.00)
    average_utilization_rate: float = Field(..., description="平均使用率", example=10.0)
    cards_expiring_soon: int = Field(..., description="即将过期卡片数", example=1)
...
class CreditCardQueryParams(BaseModel):
    """信用卡查询参数"""
    keyword: str = Field("", description="搜索关键词，支持卡片名称、银行名称模糊搜索", example="招商")
    status: Optional[str] = Field(None, description="状态筛选", example="active")
    bank_id: Optional[UUID] = Field(None, description="银行ID筛选")
    card_type: Optional[str] = Field(None, description="卡片类型筛选", example="credit")
    is_primary: Optional[bool] = Field(None, description="是否主卡筛选", example=True)
    expiring_soon: Optional[bool] = Field(None, description="是否即将过期", example=False)
...
class CreditCardBatchResponse(BaseModel):
    """批量操作响应模型"""
    success_count: int = Field(..., description="成功数量", example=3)
    failed_count: int = Field(..., description="失败数量", example=1)
