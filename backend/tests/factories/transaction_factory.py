...
def build_transaction(**kwargs) -> Dict[str, Any]:
    ...
    """
    # 随机交易金额
    amount = kwargs.get("amount") or Decimal(str(random.uniform(10.0, 5000.0)))
    ...
    cashback_earned = kwargs.get("cashback_earned") or amount * Decimal("0.01")
...
def build_large_transaction(**kwargs) -> Dict[str, Any]:
    ...
    base_transaction.update({
        "amount": kwargs.get("amount", Decimal(str(random.uniform(5000.0, 50000.0)))),
        ...
        "cashback_earned": kwargs.get("cashback_earned") or base_transaction["amount"] * Decimal("0.02")
    })
...
def build_refund_transaction(**kwargs) -> Dict[str, Any]:
    ...
    base_transaction.update({
        ...
        "cashback_earned": kwargs.get("cashback_earned", Decimal("0.00"))
    })
...
def build_transactions_batch(count: int = 5, **kwargs) -> list[Dict[str, Any]]:
    ...
    for i in range(count):
        ...
        if "transaction_date" not in transaction_kwargs:
            days_ago = random.randint(0, 30)
            transaction_kwargs["transaction_date"] = datetime.now() - timedelta(days=days_ago)
